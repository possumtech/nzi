...
function M.execute_current_line()
  ...
  local selection_tag = string.format("<agent:selection file=\"%s\" line=\"%d\" end_line=\"%d\" instruction=\"%s\">\n</agent:selection>", file_name, row, row, content);
  if type == "question" then M.handle_question(selection_tag, false)
  elseif type == "shell" then shell.run(content)
  elseif type == "directive" then M.run_loop(selection_tag, "directive", false, file_name)
  elseif type == "command" then require("nzi.commands").run(content) end
end
...
function M.execute_range(start_line, end_line)
  ...
  for i, line in ipairs(lines) do
    ...
    if type then
      ...
      local tag = string.format("<agent:selection file=\"%s\" line=\"%d\" end_line=\"%d\" instruction=\"%s\">\n%s\n</agent:selection>", file_name, start_line, end_line, instruction, selection_text);
      if type == "question" then M.handle_question(tag, false)
      elseif type == "shell" then shell.run(content)
      elseif type == "directive" then M.run_loop(tag, "directive", false, file_name)
      elseif type == "command" then require("nzi.commands").run(content) end
      return
    end
  end
  ...
  vim.ui.input({ prompt = "AI Question: " }, function(input)
    if input and input ~= "" then
      local tag = string.format("<agent:selection file=\"%s\" line=\"%d\" end_line=\"%d\" instruction=\"%s\">\n%s\n</agent:selection>", file_name, start_line, end_line, input, selection_text);
      M.handle_question(tag, false)
    end
  end)
end
...
function M.dispatch(args)
  ...
  if input == "" then
    if args.range > 0 then M.execute_range(args.line1, args.line2)
    else M.execute_current_line() end
  elseif input:match("^!") then shell.run(input:sub(2):gsub("^%s*", ""))
  elseif input:match("^[:?]") then
    local type, content = parser.parse_line("AI" .. input);
    if type == "question" then M.handle_question(content, true)
    elseif type == "directive" then M.run_loop(content, "directive", true, vim.fn.fnamemodify(0, ":.")) end
  else M.handle_question(input, false) end
end
